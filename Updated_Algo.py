...
def encode(input_filepath,text,output_filepath,avg,password=None,progressBar=None):
    ...
    k1=avg
    if avg<=(encoding_capacity-data_length):
        k2=(encoding_capacity-avg)%data_length
        i=k2
        j=k2
    else:
        i=k1
...
if __name__ == "__main__":
    ...
    if ch == 1:
        ...
        try:
            ...
            #con_ip_file = con(ip_file)
            
            #compressed = cv2.imread(con_ip_file)
            
            #print(f"PSNR value is {value} dB")
            Encoded = encode(ip_file,text,op_file,aveg,pwd)
            print(Encoded)
            #im3=cv2.imread(op_file)
